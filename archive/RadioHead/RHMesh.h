...
public:
  ...
  //?
	...
	uint16_t             dest;    ///< The address of the destination node whose route is being sought
  //?
	...
	uint16_t             dest; ///< The address of the destination towards which the route failed
    } MeshRouteFailureMessage;

    /// Constructor.
    /// \param[in] driver The RadioHead driver to use to transport messages.
    /// \param[in] thisAddress The address to assign to this node. Defaults to 0
    RHMesh(RHGenericDriver& driver, uint16_t thisAddress = 0);
    ...
    uint8_t sendtoWait(uint8_t* buf, uint8_t len, uint16_t dest, uint8_t flags = 0);
    ...
    /// \param[in] source If present and not NULL, the referenced uint16_t will be set to the SOURCE address
    /// \param[in] dest If present and not NULL, the referenced uint16_t will be set to the DEST address
    ...
    bool recvfromAck(uint8_t* buf, uint8_t* len, uint16_t* source = NULL, uint16_t* dest = NULL, uint8_t* id = NULL, uint8_t* flags = NULL);
    ...
    bool recvfromAckTimeout(uint8_t* buf, uint8_t* len,  uint16_t timeout, uint16_t* source = NULL, uint16_t* dest = NULL, uint8_t* id = NULL, uint8_t* flags = NULL);
...
protected:
    ...
    virtual bool doArp(uint16_t address);
    ...
    virtual bool isPhysicalAddress(uint16_t* address, uint8_t addresslen);
