...
/// \b TO The node address that the message is being sent to (broadcast RH_BROADCAST_ADDRESS (65536) is permitted)<br>
...
public:
    ...
    RHDatagram(RHGenericDriver& driver, uint16_t thisAddress = 0);
    ...
    bool sendto(uint8_t* buf, uint8_t len, uint16_t address);
    ...
    /// \param[in] from If present and not NULL, the referenced uint16_t will be set to the FROM address
    /// \param[in] to If present and not NULL, the referenced uint16_t will be set to the TO address
    ...
    bool recvfrom(uint8_t* buf, uint8_t* len, uint16_t* from = NULL, uint16_t* to = NULL, uint8_t* id = NULL, uint8_t* flags = NULL);
