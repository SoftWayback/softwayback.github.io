...
public:
  ...
  //?
    ...
    RHRouter(RHGenericDriver& driver, uint16_t thisAddress = 0);
    ...
    void addRouteTo(uint16_t dest, uint8_t next_hop, uint8_t state = Valid);
    ...
    ///           (usually because it dod not acknowledge due to being off the air or out of range
    uint8_t sendtoFromSourceWait(uint8_t* buf, uint8_t len, uint16_t dest, uint16_t source, uint8_t flags = 0);
