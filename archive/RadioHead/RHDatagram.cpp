...
RHDatagram::RHDatagram(RHGenericDriver& driver, uint16_t thisAddress)
    :
    _driver(driver),
...
void RHDatagram::setThisAddress(uint16_t thisAddress)
{
...
bool RHDatagram::sendto(uint8_t* buf, uint8_t len, uint16_t address)
{
    setHeaderTo(address);
    return _driver.send(buf, len);
}

bool RHDatagram::recvfrom(uint8_t* buf, uint8_t* len, uint16_t* from, uint16_t* to, uint8_t* id, uint8_t* flags)
{
...
uint16_t RHDatagram::thisAddress()
{
    return _thisAddress;
}

void RHDatagram::setHeaderTo(uint16_t to)
{
    _driver.setHeaderTo(to);
}

void RHDatagram::setHeaderFrom(uint16_t from)
{
...
uint16_t RHDatagram::headerTo()
{
    return _driver.headerTo();
}

uint16_t RHDatagram::headerFrom()
{
