...
void RH_RF95::validateRxBuf()
{
  //?
    ...
    // _rxHeaderTo    = _buf[0];
    _rxHeaderTo    = ((uint16_t)_buf[0] << 8) | _buf[1];
    // _rxHeaderFrom  = _buf[1];
    _rxHeaderFrom  = ((uint16_t)_buf[2] << 8) | _buf[3];
    _rxHeaderId    = _buf[4];
    _rxHeaderFlags = _buf[5];
    if (_promiscuous ||
...
bool RH_RF95::send(const uint8_t* data, uint8_t len)
{
    ...
    // The headers
    //?
    spiWrite(RH_RF95_REG_00_FIFO, (uint8_t)(_txHeaderTo >> 8));
    spiWrite(RH_RF95_REG_00_FIFO, (uint8_t)_txHeaderTo);
    spiWrite(RH_RF95_REG_00_FIFO, (uint8_t)(_txHeaderFrom >> 8));
