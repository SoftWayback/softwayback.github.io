...
public:
    ...
    RHReliableDatagram(RHGenericDriver& driver, uint16_t thisAddress = 0);
    ...
    bool sendtoWait(uint8_t* buf, uint8_t len, uint16_t address);
    ...
    /// \param[in] from If present and not NULL, the referenced uint16_t will be set to the SRC address
    /// \param[in] to If present and not NULL, the referenced uint16_t will be set to the DEST address
    ...
    bool recvfromAck(uint8_t* buf, uint8_t* len, uint16_t* from = NULL, uint16_t* to = NULL, uint8_t* id = NULL, uint8_t* flags = NULL);
    ...
    /// (not just those addressed to this node).
    /// \return true if a valid message was copied to buf
    bool recvfromAckTimeout(uint8_t* buf, uint8_t* len,  uint16_t timeout, uint16_t* from = NULL, uint16_t* to = NULL, uint8_t* id = NULL, uint8_t* flags = NULL);
...
protected:
    ...
    void acknowledge(uint8_t id, uint16_t from);
