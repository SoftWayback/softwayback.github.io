...
public:
    ...
    virtual void setThisAddress(uint16_t thisAddress);

    /// Sets the TO header to be sent in all subsequent messages
    /// \param[in] to The new TO header value
    virtual void           setHeaderTo(uint16_t to);

    /// Sets the FROM header to be sent in all subsequent messages
    /// \param[in] from The new FROM header value
    virtual void           setHeaderFrom(uint16_t from);
    ...
    virtual uint16_t        headerTo();

    /// Returns the FROM header of the last received message
    /// \return The FROM header
    virtual uint16_t        headerFrom();
...
protected:
    ...
    volatile uint16_t    _rxHeaderTo;

    /// FROM header in the last received mesasge
    volatile uint16_t    _rxHeaderFrom;
    ...
    uint16_t             _txHeaderTo;

    /// FROM header to send in all messages
    uint16_t             _txHeaderFrom;
